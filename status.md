...
| **In Arbeit (25.09.2026): Crunchyroll-Wochenprogramm in den Bau** | 5 | PoC durch (docs/wissen/quellen.md): Leser `pipeline/scrape-crunchyroll-woche.ts`, 16/16 Synchro-Zeilen zugeordnet, 11 mit unbekannten Folgen, **eine Zeile falsch** (Schleim S4 „Folge 22" statt 21). Offen: Regel für den Einbau mit Daniel festlegen (Vorschlag: nur künftige Termine ergänzen, nie Gemessenes überstimmen, Anschluss an die letzte gemessene Folge prüfen), dann Lauf in `refresh-hourly.yml` und Übernahme im Bau |
