...
| **Staffel 2 wird als Staffel 1 gemeldet — drei belegte Fälle** | 5 | Berichte in `docs/diagnose/`: `shangri-la-staffel2-als-staffel1.json`, `space-dandy-staffel2-ohne-folgen.json`, `sekirei-staffel2-als-staffel1.json` (alle 4.0.26, 31.08.2026). **Immer dasselbe Muster:** Adresse trägt `?ref_=atv_dp_season_select_s2`, `lage.staffelZahl` sagt 2, das Auswahlfeld zeigt den Staffel-2-Namen — und `zustand.staffelNummer` steht auf **1**, `letzteKennung` lautet `"2|1|<ASIN>|<Titel>"`. `quelltextVeraltet: false`. Seit 2.8 schlägt `headerDetail.seasonNumber` die Adresse (Yu-Gi-Oh! ZEXAL trug 101 für Band 1); hier liefert der Block offenbar die Serien- statt der Staffelnummer. **Zu prüfen:** trägt der als `isSelected` markierte Eintrag der Staffelliste die richtige Nummer? **Zusatzbefund bei Space Dandy:** Staffel 2 zeigt gar keine Folgen (`folgenImDom: 0`), der Zählstand trägt trotzdem 26 — unter der **neuen** Adresse. Der Stand der alten Staffel wandert also mit, was `fuerAdresse` verhindern soll. **Bei Sekirei:** 14 gelesene Nummern bei 13 laut Seite — derselbe Überhang wie bei Pokémon. **Nach dem Fix:** Daniel die betroffenen Anime zum erneuten Prüfen auflisten (er hat Sekirei bewusst nicht gemeldet). |
