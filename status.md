...
| **Erledigt am 16.09.2026: Crunchyroll-Verweise auf eine Reihe, die das Werk nicht führt** | 3 | `data:cr-offene` erkennt jetzt „tot", wenn die Staffelliste vollständig ist (Summe = Katalogzahl), keine Staffel die Art oder (TV/ONA) das Startjahr trägt und die Suche nichts anderes nennt (`lib/cr-reihe.ts`, Zusicherungen in `check:logic`). Gegenprobe über 40 handbelegte Titel: kein falsches „tot", 31 Urteile deckungsgleich. **Offen:** Der Lauf braucht einen deutschen Abruf und läuft nur von hier. Eine Cloudflare-Weiche wurde am 16.09. gemessen und verworfen (CLAUDE.md) — ein Cloud-Weg geht nur über Proxy oder Kleinserver (kostet Geld) |
