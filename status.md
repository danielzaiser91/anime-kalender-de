...
| **Offen (23.09.2026): Ein Crunchyroll-Block deckt zwei unserer Titel** | 2 | „Sword Art Online Alicization War of Underworld" steht bei Crunchyroll als **ein** Block mit 23 deutschen Folgen; unser Bestand führt Teil 1 (12, AniList 108759) und Teil 2 (11, 114308). Die Zuordnung sucht einen Block mit 11 Folgen, findet keinen — Teil 2 bleibt ohne Sprachurteil und steht als Crunchyroll-Aufgabe in der Statusanzeige, obwohl es dort keinen Melder gibt. Zu bauen: Deckt die Folgenzahl eines Blocks die Summe zweier aufeinanderfolgender Titel, belegt er beide mit Bereichen (dasselbe Muster wie `staffelBloecke()` bei ADN). Vorher messen, wie viele Blöcke das betrifft, und die Zuordnung mit einer Zusicherung sichern. |
| **Offen (Daniel, 23.09.2026, 00:19): Premiere-Fähnchen verschiebt die Kalenderkachel** | 1 | Eine Kachel mit Fähnchen dockt oben tiefer an als eine ohne: Am 22.09. steht Daima mehrere Pixel unter der Skeleton-Kachel vom 21.09., und in der Mo-Spalte klafft zwischen beiden Kacheln zu viel Luft. Vermutung (Daniels und meine): der Wrapper `relative mt-3` in `web/src/components/EventCard.tsx`, der dem Fähnchen Platz schafft — das Fähnchen liegt ohnehin `absolute` über der Kante. Zu prüfen: Wrapper ohne `mt-3`, Fähnchen wie in der Pille über `-top-[11px]`; danach Bild der Wochenansicht mit und ohne Fähnchen gegeneinander halten (`npm run check:ansichten`). |
| **Offen (Daniel, 23.09.2026, 00:17): Ohne deutsche Synchro in der Reihenliste sichtbar machen** | 2 | „Super Dragon Ball Heroes" steht in der Reihe von Dragon Ball Super ohne Synchro, zu sehen ist das erst nach dem Klick. Gewünscht sind **mehrere Entwürfe** als Bild (Abzeichen, gedämpfte Zeile, eigener Abschnitt, Symbol) zur Auswahl. Der Schalter „1 ohne Synchro ausblenden" bleibt davon unberührt. |
