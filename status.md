...
| Aufgabe | SP | Notiz |
|---|---|---|
| **Performance-Analyse der Datenbanksuche** | 3 | Daniel, 12.09.2026: „ich hab gerade was gesucht und es hat extrem gelaggt, sodass tastatur eingaben verschluckt wurden." Der Eingabepuffer ist eingebaut und gemessen (`npm run mess:suche --katalog`: median 82 → 13 ms, Ausreißer 534 → 24 ms). **Offen bleibt die Ursache dahinter:** Warum blockiert eine Filterung 534 ms? Zu prüfen sind die Kandidaten der Reihe nach, jeder mit Messung — (1) `useMemo` über 18.000 Titel, das bei jedem Filterwechsel neu läuft; (2) die Suche selbst, die je Titel über Titel, Studio, Genres und Keywords geht, ohne vorbereiteten Index; (3) die Zahl der gezeichneten Karten (`PAGE_SIZE`) und ob Bilder dabei synchron dekodiert werden; (4) ob die Sortierung bei jedem Anschlag neu läuft. Ergebnis und Messwerte nach `docs/analyse-suche.md`, Änderungen erst danach |
| **Analyse: Was würde eine Erweiterung um Cartoons bedeuten?** | 5 | Daniel, 12.09.2026: „wieso haben wir avatar: Herr der Elemente nicht in der datenbank? Und ‚AVATAR: DIE SIEBEN HÄFEN‘ — das ist ein anime der nächsten monat (9. Oktober 2026) rauskommt … The Mighty Nein — den führen wir auch nicht, einer meiner Lieblinge. Liegt es daran, dass diese nicht offiziell als Anime gelten? … pack auf die todo Analyse (inkl websearch) Task (Ergebnis in eine .md schreiben) was es bedeuten würde die Webseite um Cartoons, oder wie auch immer solche Serien genannt werden, zu erweitern." **Zu beantworten:** Wo genau verläuft die Grenze (AniList führt „Avatar" gar nicht — westliche Produktionen fehlen dort, TMDB kennt sie), welche Quelle trüge den neuen Bestand, wie viele Titel kämen dazu, was passiert mit dem Namen und dem Versprechen der Seite („alles mit deutscher Synchro" gilt weiter, „Anime-Kalender" nicht mehr), und welche Zwischenwege es gibt (eigener Bereich, Filter, getrennte Sichtbarkeit). Ergebnis als `docs/analyse-cartoons.md` |
