...
Stand: 22.08.2026 · Live: https://anime-kalender.de/
...
| Aufgabe | SP | Notiz |
|---|---|---|
| **Folgenbereiche sichtbar machen** | 2 | `dubRanges` steht seit dem 22.08.2026 in `data/dub-confirmed.yaml` und wird aus Daniels Einzelmeldungen gebildet (`pipeline/lib/folgenbereiche.ts`, acht Zusicherungen in `check:logic`). Gelesen wird es noch von niemandem: `loadDubChecks` reicht es durch, die Kachel zeigt weiter nur „deutsche Synchro vorhanden". **Fehlt:** eine Zeile im Detail-Panel der Art „Folgen 1–155 mit deutschem Ton, 156–171 ohne", und dieselbe Angabe im Filter „nur mit Synchro" — heute fällt Black Clover dort ganz hinein, obwohl die letzten sechzehn Folgen nicht deutsch sind |
| **Crunchyroll aus deutscher Sicht abrufen** | 5 | **Der Lauf vom 21.08.2026 hat US-Daten geliefert** — alle 1.655 geprüften Folgen in 592 archivierten Serien tragen `eligible_region: "US"`, weil GitHub-Runner in den USA stehen. Folge: Ein `de-DE` in `versions` bleibt ein Beleg (226 Serien, Kontrollgruppe 24/24), ein **fehlendes** belegt nichts — „Fairy Tail" hat in Deutschland 277 deutsche Folgen und in der US-Antwort keine. Ebenso die Verfügbarkeit: „Dragon Ball" und „Dragon Ball Z" liefern volle Folgenlisten, während die deutsche Seite das Banner zeigt. **Schritt 1 erledigt und negativ (22.08.2026): Ohne Konto geht es nicht** — Messbericht in [`docs/messung-crunchyroll-region.md`](docs/messung-crunchyroll-region.md). Vierzehn Parameter- und Header-Varianten am Prüfstein „Fairy Tail", kein einziger Treffer; `locale=de-DE` übersetzt die Texte, nicht den Katalog. Die Region steckt im **signierten** Token (`"country":"US"`, auch im JWT-Nutzteil) und im **CloudFront-signierten** CMS-Bucket `/US/M2/-` — der Pfad auf `/DE/` gebogen antwortet 403. Auch der ältere CMS-Pfad kennt nur sechs Staffeln in `ja-JP`/`en-US`. **Nächster Schritt, ein einziger Abruf:** `POST /auth/v1/token` mit Daniels Zugangsdaten vom US-Runner — kommt `"country":"DE"` zurück, entscheidet das Konto und ein hinterlegtes Anmeldegeheimnis genügt; kommt `"US"`, entscheidet allein die IP, und dann bleiben selbst gehosteter Runner, deutscher Proxy (muss Cloudflares TLS-Prüfung überstehen) oder Weiterleben mit dem US-Katalog |
...
| **46 Arbeitspakete aus `data/dub-batches.md`** | 5 | Je 20 Netflix-Zeilen mit Verweis, erzeugt aus dem Streaming-API-Lauf. **Frei seit dem Merge von PR #6 (21.08.2026).** Vor dem Vorlegen neu erzeugen: Der neue Bestand hat 488 Adressen beantwortet, die vorher offen waren — ein Teil der 56 Pakete dürfte sich damit erledigt haben |
