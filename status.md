...
| **Statusanzeige: zwei Reste** | 1 | **Am 21.08.2026 grundlegend umgebaut** — die Läufe melden ihren Zustand jetzt selbst an den Worker-Endpunkt `/lauf`, statt dass die Anzeige die GitHub-API abfragt. Damit ist der ursprüngliche Punkt „mehrere Repos gleichzeitig" gegenstandslos: Jedes Repo, das `tools/lauf-melden.sh` einbindet, meldet an dieselbe Adresse, ohne dass in der Anzeige etwas einzutragen wäre. Offen bleiben zwei Kleinigkeiten: **ein Fenster, das oben bleibt** (der Chrome-App-Modus kann das nicht von sich aus), und **eine Fassung fürs Handy** — die wäre jetzt leicht, weil die Datei nur noch eine einzige Adresse abfragt und keinen Schlüssel braucht; sie müsste nur irgendwo erreichbar liegen |
