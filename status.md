...
| Aufgabe | SP | Notiz |
| **Erledigt am 23.09.2026: `youtube-check` seit 9 Tagen stumm — Plausibilitätsriegel hielt echten Bestand für unplausibel (Lauf 35855024971, Issue #215)** | 2 | `check-youtube.ts` prüfte am 21.09. tatsächlich alle 514 fälligen Adressen (0 Störungen), fand 363 gesperrte — fast identisch zum längst committeten Bestand (362 von 514, 70,4 %). Der Riegel vom 17.09. maß aber `(leer + stoerung) / geprueft > 0.5` und hielt diesen stabilen, echten Zustand für einen Ausfall; `recordSource()` wurde im Abbruchzweig nie aufgerufen, `lastOk` fror auf dem 14.09. ein. Fix: Riegel misst nur noch die echte Ausfallquote `stoerung / geprueft` (Störungen = API hat gar nicht geantwortet), `leer` (beantwortete „in DE gesperrt"-Befunde) zählt nicht mehr mit; im Abbruchzweig jetzt ein `recordSource(..., 0, …)` nach dem Muster von `fetch-justwatch-audio.ts`. **Zu prüfen bleibt:** nächster `npm run data:check` sollte `youtube-check: ok` zeigen, und der nächste Wochenlauf sollte `data/youtube-check.json` wieder schreiben. |
...
| **Beobachten (23.09.2026): `cr-einzelwerke`/`cr-filmbloecke` seit 21 Tagen (Frist erreicht) — Nachholauf nur von Daniels Rechner möglich** | 1 | Bau-Lauf 35855024971 meldete beide Quellen exakt an ihrer 21-Tage-Frist als stumm (`check-sources.ts`: „laufen nur von Daniels Rechner … Frist großzügig, drei Wochen"). Kein Code-Fehler — die Quellen brauchen eine deutsche Leitung. Griff bei Gelegenheit: `npm run data:cr-einzelwerke && npm run data:cr-filmbloecke` von Daniels Rechner aus. Kein PR. |
