...
| **Beobachten (23.09.2026): aniSearch sperrt seit mind. vier Tagen (HTTP 423) — inzwischen vier rote Bau-Läufe an einem Tag** | 1 | Vier Tage in Folge (20.–23.09.) bricht `npm run data:anisearch` sofort mit `HTTP 423` auf den ersten Kennungen ab, danach `fetch failed` für den Rest des Laufs — kein Selektorfehler (keine Codeänderung an den beteiligten Dateien seit über zehn Tagen), sondern eine Sperre bei aniSearch selbst, wie schon am 09./10.08.2026. `check-sources.ts` bricht deshalb zu Recht den Bau-Lauf ab (Issue #214) — nachgetragen 15:32 Uhr: Lauf 35881823996 ist bereits der **vierte** rote Bau-Lauf an diesem Tag (09:28, 11:52, 13:xx, 15:28) an derselben Stelle, nichts Neues an der Ursache. Nichts an der Quelle zu reparieren, kein PR dafür. **Zweite Hälfte gefunden und Fix bereit, aber blockiert (Issue #218):** Ein roter Bau-Lauf ließ `deploy.yml` bisher komplett aussetzen, auch wenn der Schritt „Änderungen committen" längst durchgelaufen war (Commit `640fc2b` blieb so unausgeliefert). Der fertige Patch (neuer Job `pruefe-ausloeser`, prüft gezielt jenen Schritt statt der Gesamtnote) steht als Diff im Issue #218-Kommentar — `check:vor-commit` lokal grün, aber der Push scheitert am `claude[bot]`-Token: „refusing to allow a GitHub App to create or update workflow … without `workflows` permission". **Braucht Daniel:** den Patch aus Issue #218 von Hand in `.github/workflows/deploy.yml` einspielen. Zu prüfen bleibt außerdem: nächster `npm run data:check` — steht `anisearch: ok` wieder im Protokoll? |
