...
| **Ein Quellen-Widerspruch: „Kill Ao" (198113)** | 1 | `npm run check:quellen` meldet einen falsch negativen Befund: Die Quelle sagt „keine deutsche Tonspur", die Handprüfung sagt deutsch. Ein einzelner Datenpunkt, kein Systemfehler — 96 % der verglichenen Urteile sind einig. Die Prüfung gehört nicht zu `check:vor-commit` und macht keinen Lauf rot. Braucht einen Blick, welche Seite recht hat |
| **26 Crunchyroll-Verweise ohne Urteil — überwiegend Filme** | 5 | Von 34 sind am 09.09.2026 acht geklärt (`pipeline/fetch-crunchyroll-offene.ts`, wöchentlich im Tiefendurchlauf): fünf Sprachurteile über Videokennung und Staffelliste, drei abgelaufene Videokennungen. **Was übrig bleibt, ist eine Namensfrage:** Crunchyroll führt Filme unter Kurznamen — „Fruits Basket -prelude-" heißt dort „-prelude-", „Free! -Timeless Medley- Kizuna" heißt „The Bond". Über den Titel ist einer von 23 zuzuordnen, und ein lockerer Vergleich wäre falsch (fünf „Free!"-Filme zeigen auf die Serienadresse ihrer Reihe). **Nächster Ansatz:** `discover/search?type=movie_listing` liefert 69 Filme mit 44 deutschen Tonspuren — sie über das **Jahr** und die Laufzeit zuzuordnen statt über den Namen ist noch nicht gemessen |
| **Death Note: Relight — zwei Prime-Ausgaben, ein Eintrag** | 2 | Daniel am 09.09.2026: Prime führt „Relight 1: Visions of a God" (B0FVDZ286F) und „Relight 2: L's Successors" (B0FWYWSS3M) als zwei Kauftitel; unser Eintrag 2994 hat zwei Folgen und meint beide. Die Erweiterung kann es seit 4.16.1 (Ankreuzen auf der Suchseite, Checkliste auf der Titelseite). **Gemessen ist die Beleg-Ebene:** `wegAusMeldung()` in `fetch-pruefungen.ts` baut aus Suchadresse und `seiten_kennung` je Meldung eine eigene Adresse, und `loadDubChecks()` trennt seit dem 07.09.2026 je Ausgabe — zwei Belege entstehen also sauber. **Nicht gemessen:** ob daraus zwei **Verweise** am Titel werden. Das lässt sich erst prüfen, wenn Daniels beide Meldungen da sind |
