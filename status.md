...
| **22 Crunchyroll-Verweise ohne Urteil — fast nur Filme** | 3 | Stand 09.09.2026, 19:45: Von 31 geprüften Adressen sind 6 beurteilt und 3 tot. **Drei Wege sind seit heute gebaut** (`fetch-crunchyroll-offene.ts`): der **Slug** aus unserer Adresse führt zur Serienkennung (auf beiden Seiten normalisiert — der Katalog schreibt `free---iwatobi-swim-club`), ein Slug auf einen **Filmeintrag** ist das Werk selbst und nicht die Reihe, und **einstimmige Staffeln** brauchen keine Zuordnung („Meine Wiedergeburt als Schleim": sechs Staffeln, alle deutsch — drei Verweise auf einen Schlag). Der Katalog holt seit heute auch `type=movie_listing` (69 Filme, 44 mit de-DE); ohne den Parameter liefert `browse` nur Serien, und `it.type` sagt bei allen „series". **Was offen bleibt, ist es zu Recht:** 8 der 22 sind „Free!"- und Chunibyo-Ausgaben, deren Adresse auf die **Reihe** zeigt — die vererbt ihre Sprache nicht. Weitere 6 Filme („Liz and the Blue Bird", „Okko's Inn", „Millennium Actress", „Cencoroll", „Hamatora") stehen im deutschen Katalog gar nicht. Ein Urteil bräuchte die Adresse der deutschen Fassung, und die kennt niemand |
...
| **1 ADN-Verweis ohne Urteil — „High Speed! Free! Starting Days"** | 1 | Am 09.09.2026 waren es vier; drei sind über `/show?limit=100&offset=…` geklärt (171 Shows über drei Seiten — die API meldet weiter `total: 252`): beide One-Piece-Filme führen `vde`, „Plus-Sized Elf" ausschließlich `vostde`. **Zweimal eingetragen:** Der erste Eintrag wurde eine Stunde später von einem Datenlauf überschrieben — `commit-data.sh` setzt hart auf `origin/main` zurück und spielt seine eigenen Quellen wieder ein (CLAUDE.md, 29.08.2026). Drei Zusicherungen in `check:logic` halten die Zeilen jetzt namentlich fest. Übrig ist der Slug `free-starting-days`, zu dem es unter den 171 Shows keinen Treffer gibt |
