...
| **Erledigt am 16.09.2026: Crunchyroll-Verweise auf eine Reihe, die das Werk nicht führt** | 3 | `data:cr-offene` erkennt jetzt „tot", wenn die Staffelliste vollständig ist (Summe = Katalogzahl), keine Staffel die Art oder (TV/ONA) das Startjahr trägt und die Suche nichts anderes nennt (`lib/cr-reihe.ts`, Zusicherungen in `check:logic`). Gegenprobe über 40 handbelegte Titel: kein falsches „tot", 31 Urteile deckungsgleich. Läuft seit 16.09.2026 in der Cloud über die Vercel-Weiche in Frankfurt (`weiche-vercel/`, Wochenlauf und `daten-auf-abruf`); die Cloudflare-Weiche davor trug nicht (CLAUDE.md). **Offen:** `CR_ZUGANG` ebenso über die Weiche erneuern |
