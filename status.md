...
| Aufgabe | SP | Notiz |
| **Warten auf Feedback (26.09.2026): UI-Überarbeitung, Prototyp B „Poster“ Runde 3** | 8 | Leinwand https://claude.ai/artifact/AEJUirTAgvme2kH4m8uHpH (Boards `B-Poster`, `B-Monat`, `B-Handy`), noch nicht in `web/src`; erste Runde steht in PR #244 (Zweig `claude/loving-einstein-997iiv`). Entscheidungen mit Daniel: Agenda-Reiter entfällt; Favoriten und „Wo sehen?“ entfallen als Reiter, weil der Filter „Nur Favoriten“ und Anbieter-Chips beides abdecken (Push-Schalter wandert ins Abo-Menü, AniList-Import unter „Nur Favoriten“; offen für den echten Umbau: wohin „gesehen bis Folge N“ aus `FavoritesView` geht, Vorschlag Detail-Panel); Navigation Kalender (Woche ⇄ Monat) · Datenbank · News; Abo und Newsletter in einem Knopf, Beschriftung nur Kalender- und Glockensymbol; „Demnächst: Staffelstarts“ gestrichen (Monatsansicht und breite Staffelstart-Karten zeigen dasselbe, im Oktober wären es zwei Einträge). Neu im Prototyp: Staffelfinale-Karte (Regel: `episode === episodeCount`, wöchentlich, kein `episodeCountAssumed`, nicht `available-from`), Kacheln an die Breite gekoppelt (Handy genau zwei je Zeile), TV-Kasten scrollt ab Höhe der Poster-Reihe, Logo im Kopf, Zahnrad und Hell/Dunkel zurück, AniList-Cover statt OG-Ausschnitten |
...
| **Queue (26.09.2026): JustWatch-Abgleich für Titel ohne Treffer** | 3 | Daniel am 26.09.2026: ja. 646 Titel ohne JustWatch-Treffer, 17 verlieren dadurch Bezugswege (Stand 23.09.). Erst messen: Stichprobe von 30 Titeln von Hand bei JustWatch suchen und festhalten, woran der Abgleich scheitert (Schreibweise, Untertitel, Jahr), dann den Abgleich ändern. |
...
| **Meldemodell in vier Stufen (docs/konzept-meldungen-architektur.md)** | 13 | **Stand 26.09.2026:** Stufe 1 (22.09.), 2 und 3 (22.09., laufen im Bestandslauf) und Stufe 4 Schritt 1 (23.09., Urteil füllt Lücken) sind umgesetzt; die Durchgänge seit 4.21 haben Stufe 1 praktisch getestet. Neu gemessen 26.09.2026: 6.617 Urteile (6.074 deutsch, 134 kein Deutsch, 409 unbekannt = Kanal ohne Abo) an 301 Wegen — **0 Lücken, 0 Widersprüche, 0 Wege nur mit Nein, 0 gemischt ohne Bereiche**. Schritt 2 hat damit weiter keine Arbeit. **Offen, Schritt 3:** die 26 Setzstellen in `build.ts` durch das Urteil ersetzen (Quelle für Quelle, `check:handbelege` als Gegenprobe), Pill-Achse (welcher Verweis gehört zur Folge), Nachprüfliste aus `unbekannt` und Widersprüchen. **Danach:** Wiedervorlage eigener Prüfungen nach 180 Tagen, Statistik je Quelle, Protokoll „Aussage zuerst gesehen“, `befund`-Ableitung im Worker streichen, Kanal-Regel in CLAUDE.md und quellen.md nachziehen. |
