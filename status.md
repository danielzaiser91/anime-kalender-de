...
| **1 ADN-Verweis ohne Urteil — „High Speed! Free! Starting Days"** | 1 | Am 09.09.2026 waren es vier; drei sind über `/show?limit=100&offset=…` geklärt (drei Sammelseiten, 171 Shows — die API meldet weiter `total: 252`): beide One-Piece-Filme führen `vde`, „Plus-Sized Elf" ausschließlich `vostde`. Die Kennungen stehen in `data/adn-adressen.yaml`. Übrig ist ein Verweis mit dem Slug `free-starting-days`, zu dem es unter den 171 Shows keinen Treffer auf „Free" oder „Starting Days" gibt — und aus einem Fehlen folgt hier nichts. **Nebenbefund:** Der Bau **hier** kommt nicht bis zum Schreiben; er bricht am Titelschwund-Riegel ab, weil der lokale `data/cache/` älter ist als der des letzten Cloud-Laufs. Die Wirkung ist trotzdem gemessen — bis unmittelbar vor der Auslieferung steht `dub: true` am Verweis |
