...
| Aufgabe | SP | Notiz |
|---|---|---|
| **D1: Index nachziehen und Rohfolgen anwenden** | 1 | Wartet auf den Reset des Tageskontingents am 02.09.2026, 02:00 unserer Zeit (00:00 UTC) — ein `CREATE INDEX` liest die Tabelle selbst. Dann in dieser Reihenfolge: `worker/migrations/023-netzfund-url-index.sql` ausführen (der letzte Zugriff ohne Index, `SELECT id FROM netzfund WHERE url = ?`), danach `npx tsx pipeline/fetch-rohfolgen.ts` — es bringt die **15 neu zugeordneten Adressen** in den Bestand und misst nach, wie viele der 198 offenen Rohfolgen bleiben. Zuletzt die Wirkung des Caches messen: zwei Abrufe auf `?zaehlen=1`, der zweite muss aus der Edge kommen. Die Zeilenersparnis selbst ist gerechnet (9,9 Mio. → 331.000 am Tag), nicht gemessen |
