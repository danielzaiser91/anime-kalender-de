...
| Aufgabe | SP | Notiz |
|---|---|---|
| **Erledigt am 10.09.2026: Vorfälle der Erweiterung kommen an** | — | Daniel: „info bringt nix, du liest nix aus der console aus, ich lese auch nix aus … du musst informiert werden über issues." Die Erweiterung schrieb seit Monaten Diagnosen in die Browserkonsole — sie existierten nur, wenn er zufällig hinsah und ein Bild schickte. Gebaut ist der Weg, den die Meldungen ohnehin gehen: Migration 027 (`vorfall`), Worker-Endpunkt `/vorfall` (POST melden, GET holen, DELETE aufräumen, dieselbe Sache je Seite einmal pro Tag), `vorfallMelden()` in `melder.js` für **ohne_tonspur**, **fremde_reihe** und **stoerung**, und `pipeline/fetch-vorfaelle.ts` schreibt `daniel-zum-abarbeiten/17-vorfaelle.md`. Eingehängt in `bestand-bauen.yml`. Ausgeliefert und von beiden Seiten geprüft (Version 574bb1aa). **Offen bleibt Prime und Disney+** — dort schreibt die Erweiterung weiter nur in die Konsole |
| **Zehn Migrationen gelten als offen, obwohl ihre Tabellen produktiv sind** | 2 | Beim Ausliefern am 10.09.2026 aufgefallen: `wrangler d1 migrations list` führt **018 bis 027** unter „Migrations to be applied" — dabei liest der Worker `such_erwartung` (026) und schreibt `seiten_kennung` (020) längst. Sie wurden also von Hand über `d1 execute` angewandt, und die `d1_migrations`-Tabelle weiß nichts davon. **Die Falle:** Ein `migrations apply` würde alle zehn erneut fahren; die `CREATE TABLE IF NOT EXISTS` sind harmlos, ein `ALTER TABLE ADD COLUMN` auf eine vorhandene Spalte bricht ab — mitten im Stapel. Deshalb wurde 027 einzeln per `d1 execute` angewandt, wie offenbar alle davor. Zu klären: die `d1_migrations`-Tabelle nachtragen oder den Mechanismus bewusst aufgeben und es dokumentieren |
| **Melde-Knopf an jede Folgenzeile** | 3 | Daniel am 10.09.2026 beim Abarbeiten der Netflix-Prüfliste: Um **genau eine** Folge zu prüfen, muss er heute entweder `↻ alle` drücken (das prüft dann Anfang und Ende) oder die Folge von Hand im Player öffnen. Die Prüfliste nennt aber punktgenaue Aufträge („S1 Folge 26", „S1 F13–15"). Ein kleiner Knopf an der Folgenzeile selbst — sichtbar nur, solange für diesen Titel ein Auftrag besteht — verkürzt das auf einen Klick. Die Folgenzeilen findet der Melder bereits (`[data-testid="episode-list-item"]`) |
...
| **623 Amazon-Adressen mit offenem Befund** | 2 | Der Prüflauf vom 07.09.2026 fand 116 neue 404 und 9 Regionssperren — und danach 623 mal `unklar`, weil Amazons Abwehr nach rund 660 Abrufen zumacht (Gegenprobe: auch eine lebende Adresse kam nur noch als 3.815-Zeichen-Seite). Der Riegel arbeitet richtig: `unklar` heißt „weiß ich nicht" und bleibt fällig, statt ein falsches „lebt" zu buchen. **Zu tun ist nichts von Hand** — die wöchentlichen Läufe klären die 623 in Losen, seit der Lauf bei zwanzig Zwischenseiten in Folge abbricht statt weiterzuklopfen. Hier steht es nur, damit die Zahl nicht als Fehler gelesen wird |
| **47 Handbelege gelten der ganzen Plattform, obwohl sie an einer Seite gemessen wurden** | 2 | Gefunden am 10.09.2026: `data/dub-confirmed.yaml` führt **854** Belege, deren Notiz eine Amazon-Kennung nennt („Amazon-Seite B07L1CMH2D: alle 0 Folgen geprüft"). **497** davon tragen kein `url`-Feld — und ohne das gilt der Befund für **jede** Adresse dieser Plattform, auch für später hinzukommende. Bei 47 ist das folgenreich, denn ihr Urteil **entfernt** (`available: false` oder `dub: false`). **Der Fall ist real eingetreten:** „Haikyu!! 3rd Season" bekam am 10.09.2026 die Prime-Adresse `B0D544CDK6` (elf Folgen, gerechnet zugeordnet) — und ein Beleg vom 25.08. zu `B07L1CMH2D` („alle 0 Folgen") warf sie sofort wieder hinaus. Dort ist die Adresse jetzt als Feld nachgetragen. **Offen ist die allgemeine Fassung:** `fetch-pruefungen.ts` schreibt `url` nur, wenn die Adresse dem Datensatz unbekannt ist — dabei braucht sie der Beleg gerade dann, wenn sie bekannt ist. Die Umstellung auf „immer mitschreiben" ändert das Verhalten aller künftigen Belege und gehört deshalb besprochen, nicht nebenbei gemacht |
| **223 Titel ohne jeden Bezugsweg — alle Quellen sind gefragt, keine kennt sie** | 1 | Gemessen am 10.09.2026: 233 von 2.768 Titeln zeigen keinen einzigen Weg. Aufgeschlüsselt sind es vier Gruppen, und nur eine war zu schließen: **12** kennt aniSearch (davon 7 YouTube, die bewusst draußen bleiben — ein Kanal zeigt dort regelmäßig die untertitelte Fassung), **1** kennt JustWatch, **35** hatten einen Verweis, der belegt entfernt wurde (18 „kein deutscher Ton", 12 „Katalog führt die Staffel nicht", 8 „nicht mehr verfügbar"). Bleiben **223**, zu denen **keine** Quelle etwas führt — und sie sind nicht ungefragt: JustWatch hat sie am 09.09.2026 abgerufen und nichts gefunden. Nach Jahren: 100 vor 2000, 28 aus den 2000ern, 73 aus den 2010ern, 20 ab 2020. **Das ist keine Lücke im Ablauf, sondern in den Quellen** — die meisten dieser Titel laufen in Deutschland tatsächlich nirgends mehr. Die Wiedervorlage greift nach 28 Tagen von selbst; das Wochenlimit ist deshalb von 40 auf 200 erhöht, sonst käme der Lauf bei 259 gleichzeitig fälligen Titeln nie durch |
