...
| **Nachrichten-Bereich auf der Seite** | 3 | Daniel, 12.09.2026: „a news section for the website, where all our news regarding dubs (ankündigungen und releases) are published in a bite-sized format (short and simple)". Datenlage steht: `neu-mit-synchro.json` (Titel + Datum, speist heute den Newsletter), `crunchyroll-neu.json` (neue Folgen je Tag), Releases mit Termin, Disc- und Kino-Einträge. Offen: Entwurf, Route, Bau von `news.json` |
