...
|---|---|---|
| **Wache durchsehen** | **2026-09-20T21:25+02:00** | Sind seit diesem Zeitpunkt mehr als 24 Stunden vergangen, wird `daniel-zum-abarbeiten/00-wache.md` und `00-wache-auswertung.md` gelesen, danach der Zeitstempel hier erneuert. Beurteilt wird entlang der vier Fragen: Was läuft korrekt, wo ist Verbesserungspotenzial, wo sind echte Risiken und Lücken, was läuft komplett falsch. **Der Stempel wird nur erneuert, wenn wirklich gelesen wurde** — sonst ist er eine Zahl, die Händewaschen behauptet |
| **Crunchyroll-Zugangspaket** | **2026-09-06T08:15+02:00** | `CR_ZUGANG` lebt **24 Stunden** und kann nur an einer deutschen Leitung entstehen — der wöchentliche Lauf kann es also nie selbst frisch haben. Am 06.09.2026 war es acht Tage tot, und niemandem ist es aufgefallen: Die Frist in `check-sources.ts` misst, wann `crunchyroll-dub` zuletzt **geschrieben** hat, nicht ob der Lauf überhaupt arbeiten **kann**. Deshalb hier: Hat `data/source-health.json` für `crunchyroll-dub` seit mehr als sieben Tagen nichts gesehen, wird das Paket erneuert und der Nachholauf angestoßen (`node tools/cr-zugang-holen.mjs --secret`, dann `gh workflow run crunchyroll-nachholen.yml -f limit=0 -f alter=28`). Ein täglicher Alarm dafür wäre falsch — das Paket ist an fast jedem Tag zu Recht abgelaufen |
...
| **Queue (25.09.2026): Crunchyrolls Season-Lineup automatisch lesen** | 3 | Handpflege in `data/ankuendigungen.yaml` ist die Übergangslösung. Der Artikel `/de/news/seasonal-lineup/<jahr>/<m>/<t>/crunchyroll-anime-lineup-<season>-<jahr>` nennt je Titel `OmU: <Datum>` und `DE: TBA`; Zuordnung wie beim Handabgleich über Name **und** Japan-Start. Nächste Season: Winter 2027 (Artikel erwartungsgemäß Mitte Dezember) |
...
| **Queue (25.09.2026): Anime2You-Vorschläge einarbeiten** | 5 | 103 von 113 Vorschlägen nie angefasst, darunter 55 verschobene Blu-ray-Termine (22.09. und 01.09.). Die Quelle läuft längst, nur die Auswertung fehlt |
| **Queue (25.09.2026): Crunchyroll-Wochenprogramm — nachgereichte Synchro-Pakete** | 2 | Hana-Kimi S2 Folge 10–12 am 02.10., Polar Opposites S2 Folge 8–11 am 04.10., Mushoku Tensei S3 Folge 11–12 am 04.10. stehen nur dort. Gemessen 25.09.2026: Der Kalenderabruf (`data/crunchyroll.json`, Fenster bis 04.10.) enthält keinen Synchro-Slot nach dem 25.09. — Crunchyrolls Kalender führt künftige Synchro-Folgen nicht. Also das Wochenprogramm lesen (Artikel `…/crunchyroll-wochenprogramm-<season>-<jahr>`, montags aktualisiert, Synchro an der zweiten Folgenzeile je Titel und „*erscheint am …") |
| **Queue (25.09.2026): „▶ alle durchgehen" auch für Disney+** | 3 | Daniel: „hier wäre alles durchgehen trotzdem nice, dann kann er im hintergrund weiter sammeln, während ich tab wechsele". Die Erweiterung sammelt dort schon je Seite alle Staffeln; es fehlt nur der Lauf über die Liste (Gerüst wie beim Prime-Durchgang: Tab-Speicher, Spur, Ende im Dialog). Nach dem Prime-Umbau |
| **Offen (25.09.2026 morgens): Migrationen 037 und 038 einspielen, D1-Lesezahlen nachmessen** | 1 | Am 24.09.2026 ab 19:45 war D1s Tageskontingent (5 Mio. gelesene Zeilen) aufgebraucht — der Prüfstand las 9,2 Mio. D1 nimmt bis Mitternacht UTC keine Migration an. Danach: `wrangler d1 migrations apply anime-kalender --remote --config wrangler.toml` im Ordner `worker/`, dann `wrangler d1 insights … --timePeriod 1d` und prüfen, welche der sechs Titel des letzten Durchgangs (Assassination Classroom, My Hero Academia, Made in Abyss, Kakegurui, Shield Hero, Hochzeit) angekommen sind; Fehlendes neu vorlegen. |
...
| **Offen: PR #189 seit 21.09.2026 unmerged — deshalb Lauf 35900967510 heute mit derselben Ursache erneut rot** | 1 | `tools/commit-data.sh` sucht den Grund für die Statusanzeige mit `grep -m 3 '✖' /tmp/bestand-pruefung.txt`. `check:handbelege` (heute: vier abweichende Handbelege — Free!, Free!: Eternal Summer, AIR GEAR Special, Girls und Panzer Specials, ein eigener legitimer Befund für den nächsten Bau-Zyklus) meldet aber mit „⚠", nicht „✖". Grep findet nichts, Exit 1, und reißt als ungeschützte Variablenzuweisung unter `set -euo pipefail` das ganze Skript mit — noch bevor die `WARNUNG_STATT_ROT`-Behandlung lief, die genau diesen Fall gelb statt rot machen soll. **Der eigentliche Befund ist nicht der Bug, sondern dass er schon einmal gefunden und behoben wurde:** Issue #188/PR #189 vom 21.09.2026 (Lauf 35566086288, ausgelöst vom selben `check:handbelege`-Muster) tragen exakt denselben Fix, unverändert `MERGEABLE`/`CLEAN` gegen den aktuellen `main` — nur nie gemergt. Meine unabhängige Neudiagnose (Issue #221/PR #222) kam beim selben Fix an und wurde als Duplikat geschlossen. **Zu tun:** PR #189 mergen. Zusätzlich offen (steht im PR, nicht selbst behebbar — GitHub-App-Token darf keine Workflow-Datei pushen): dieselbe ungeschützte Grep-Zuweisung für `check:logic` in `.github/workflows/bestand-bauen.yml` Zeile 144. |
| **Offen (ab 24.09.2026): Joyn-Weg für „Dragon Ball" aufnehmen** | 2 | Die erste Serie läuft ab dem 24.09.2026 Mo–Fr ab 17:10 in Doppelfolgen auf ProSieben MAXX; die Folgen stehen danach kostenlos in der Joyn-Mediathek (rollendes Fenster, wie bei Dragon Ball Super die Folgen 110–129). Sobald die erste Folge dort steht: Weg für AniList 223 eintragen und das Fenster pflegen. Dasselbe für Z Kai, sobald es wieder läuft. Belege im Rechercheabschnitt vom 23.09.2026. **Stand 24.09.2026, 17:55:** JustWatch führt für keinen Dragon-Ball-Titel ein Joyn-Angebot (auch Super nicht mehr) — morgen erneut fragen; Joyn selbst wird nicht ausgelesen (Wissen „Joyn: nicht selbst auslesen"). |
...
| **Beobachten (23.09.2026): aniSearch sperrt seit mind. vier Tagen (HTTP 423) — inzwischen fünf rote Bau-Läufe an einem Tag** | 1 | Vier Tage in Folge (20.–23.09.) bricht `npm run data:anisearch` sofort mit `HTTP 423` auf den ersten Kennungen ab, danach `fetch failed` für den Rest des Laufs — kein Selektorfehler (keine Codeänderung an den beteiligten Dateien seit über zehn Tagen), sondern eine Sperre bei aniSearch selbst, wie schon am 09./10.08.2026. `check-sources.ts` bricht deshalb zu Recht den Bau-Lauf ab (Issue #214) — nachgetragen 19:17 Uhr: Lauf 35894552930 ist bereits der **fünfte** rote Bau-Lauf an diesem Tag (09:28, 11:52, 13:xx, 15:28, 19:17) an derselben Stelle, nichts Neues an der Ursache. Nichts an der Quelle zu reparieren, kein PR dafür. **Zweite Hälfte gefunden und Fix bereit, aber blockiert (Issue #218):** Ein roter Bau-Lauf ließ `deploy.yml` bisher komplett aussetzen, auch wenn der Schritt „Änderungen committen" längst durchgelaufen war (bestätigt auch für Lauf 35894552930: „Änderungen committen" erfolgreich, nur „Quellen auf Schweigen prüfen" rot). Der fertige Patch (neuer Job `pruefe-ausloeser`, prüft gezielt jenen Schritt statt der Gesamtnote) steht als Diff im Issue #218-Kommentar — `check:vor-commit` lokal grün, aber der Push scheitert am `claude[bot]`-Token: „refusing to allow a GitHub App to create or update workflow … without `workflows` permission". **Braucht Daniel:** den Patch aus Issue #218 von Hand in `.github/workflows/deploy.yml` einspielen. Zu prüfen bleibt außerdem: nächster `npm run data:check` — steht `anisearch: ok` wieder im Protokoll? |
...
| **Queue (23.09.2026): Anbieter-Pillen nutzen noch den Browser-Tooltip** | 1 | `Pille` in `DetailPanel.tsx` reicht ihren Hinweis als `title`-Attribut durch, nicht an den gestalteten Tooltip aus `ui.tsx` (Daniel, 12.08.2026: „keine default web tooltips … überall nutzen"). Seit dem 23.09. ist der Hinweis mehrzeilig — im Systemtooltip funktioniert das, sieht aber anders aus als überall sonst. Beim Umstellen `whitespace-pre-line` an der Blase ergänzen und die Pillenreihe im Bild gegenhalten (der Tooltip-Baustein hüllt in zwei `span`, die Reihe bricht um). |
...
| **Queue: JustWatch-Seiten ohne Klick in der Prüfliste zuordnen (19.09.2026)** | 2 | Der Klick in der Prüfliste trägt den Auftrag seit 4.20.35 über Amazons Weiterleitung (`ref_=atv_dl_rdr`), ab 4.20.36 vor der Kopf-gti; im Durchgang kamen so 18 Meldungen richtig an. Offen bleibt der Weg ohne Klick (Link aus dem Chat, Lesezeichen): Die Kopf-gti der Seite ist eine andere als JustWatchs (Air Gear 66c97aeb… gegen 4825705e…). Idee: alle gtis des Hydration-Blocks gegen die Liste halten — erst messen, ob JustWatchs gti dort überhaupt vorkommt (Diagnosebericht einer solchen Seite). **Gemessen am 21.09.2026 an A Silent Voice** (Link direkt geöffnet, nicht aus der Prüfliste): JustWatchs gti `5482d2e9…` steht an erster Stelle der 49 gtis im Quelltext, der Auftrag wurde trotzdem nicht erkannt. Der Weg trägt also in diesem Fall. Umgebaut wird erst mit einem zweiten Fall — die Prüfliste führt gerade nur diese eine JustWatch-Seite, und bei Air Gear war die Kopf-gti eine andere; ob JustWatchs gti dort ebenfalls vorn steht oder erst im Empfehlungsblock, ist offen. |
...
| **Blockiert seit 16.09.2026: Synonyme für die Katalogeinträge nachholen — aniSearch sperrt die GitHub-Läufe** | 1 | Synonyme (oft der deutsche Name) werden erst seit dem 12.09.2026 gespeichert; ältere Einträge gelten als fällig (3.489 offen). **Beide Nachhol-Läufe (35096344686, 35101202026) holten 0 Seiten**: aniSearch antwortete mit HTTP 423 (Locked), danach „fetch failed" — und der Abruf fragte trotzdem 2.400-mal weiter. Letzter erfolgreicher Abruf von GitHub aus: 14.09.2026 (Wochenlauf, 400 Seiten). Der Titelabruf lief mit 2 s Takt, dreimal über aniSearchs dokumentierter Grenze (10/min). Behoben: 6 s Takt, Abbruch bei 403/423/429 und nach fünf Fehlschlägen in Folge (auch im Folgenabruf). **Nicht erneut angestoßen.** Gemessen am 16.09.2026 von Daniels Anschluss: mit unserer Kennung (`anime-kalender.de/1.0 (+…; Mail)`) HTTP 423, mit Browser-Kennung 301 — die Sperre gilt also der **Kennung**, nicht (nur) der IP. Umgehen per Browser-Kennung scheidet aus (Tarnung, siehe Learnings 09.08.). Erste Sperre 09./10.08.2026: gewartet, nach gut einem Tag von selbst weg; dazu Takt, Kontingent, ehrliche Kennung, Mail an api@anisearch.com. Ob sie mit der Mail an aniSearch zusammenhängt, ist offen. Für die Dai-Filme führt aniSearch nur englische Synonyme — dort gibt es keinen deutschen Namen |
...
| **Kanal-Meldungen, denen JustWatch widerspricht — umgestellt am 14.09.2026** | 1 | Die fünf Fälle vom 09.09. waren überwiegend keine Widersprüche: Seit dem 14.09. vergleicht `kanal-gegenprobe.ts` mit dem Angebot des gemeldeten Kanals (CLAUDE.md, „Zwei Ausgaben derselben Staffel"). Digimon ist gelöst (deutsche Ausgabe belegt, Kanal-Ausgabe durchgestrichen), Trinity Seven ist ein belegtes Nein, Date a Live II steht nicht mehr darunter. **Offen auf Daniels Prüfliste (Stand 15.09.2026, 08:35):** Ausgaben-Suchen für Bungo Stray Dogs, Touken Ranbu und Okko; echte Widersprüche, bei denen das Crunchyroll-Kanal-Angebot selbst Deutsch hat, für Free! und Fruits Basket |
...
| **Sammeln und Zuordnen trennen — gemessen erledigt, Rest ist Kosmetik** | 2 | Daniel am 01.09.2026: „anbieter zuordnung nicht vertrauen, sondern einzeln episoden korrekt aus gesammeltem zustand rauspicken". **Gemessen am 09.09.2026, 20:15:** Beide Melder gehen die Rohfolgen-Route längst — `melder.js` (Netflix) seit dem 01.09., `disney.js` ebenso; der Worker nimmt sie seit demselben Tag je Plattform an, und `data/prime-zugeordnet.json` führt 22 Adressen, davon eine von Netflix. Die **Anbieter-Staffel entscheidet nichts mehr**: `fetch-rohfolgen.ts` liest `staffel_nr` an genau einer Stelle, und dort nur als Riegel („ab Staffel 2 keinen Reihenkopf-Treffer"). Die Zuordnung selbst läuft über Folgentitel und Erstausstrahlung aus aniSearch und TMDB. **Offen bleibt die Umbenennung** von `prime_folge` und `data/prime-zugeordnet.json` auf einen anbieterneutralen Namen — reine Lesbarkeit, dafür eine D1-Migration im laufenden Betrieb. Bewusst zurückgestellt |
...
Punkte, die nur bei Gelegenheit auftauchen und dann kurz geprüft werden. Daniel am 13.09.2026: „dann versteck ihn vom footer und merk ihn anders.“

| Was | Woran man es erkennt | Was dann zu tun ist |
|---|---|---|
| **Bereichsmelder auf Netflix** (seit 4.17.7 gebaut, nie gesehen) | Beim Prüfen einer Staffel erscheint im Kasten das Zahlenfeld „dt. bis Flg. ?“ mit „✓ melden“ — nur wenn die erste Folge deutsch ist und die letzte nicht | Daniel trägt die letzte deutsche Folge ein und meldet; ich prüfe im Briefkasten, dass zwei Bereiche angekommen sind. Konosuba S2 (13.09.2026) war durchgehend deutsch, das Feld kam deshalb nicht |
...
| Aufgabe | Notiz |
|---|---|
| **Prime-Durchgang in Frames (4.23.0) — erster Lauf ausstehend; davor Navigation 4.22.x (25.09.2026)** | Lauf 15:13–15:35 UTC: alle 5 Titel, 102 Staffelseiten gemeldet (Naruto 9, Pokémon 57 Bände, Advanced Generation 15, Diamant & Perl 15, Yu-Gi-Oh! 6), ein Hänger (4.22.8 behoben). Rund 10 s je Seite, die Hälfte eigene Wartezeit (4.22.9 streicht sie). Bände gingen mit `staffel` 201, 1001 … raus — nach dem nächsten Bestandslauf prüfen, wie der Zuordner sie legt. PoC „Staffel per Abruf lesen" (Ton 31/31) gemessen, Weg aber verworfen: Frames nutzen den bewährten Knopf (4.23.0). Offen: Tempo mit drei Frames, Drosselung im Hintergrund-Tab. Dazu am selben Tag: Netflix-Randprobe in Stapeln zu zehn (4.22.4), Film-Einträge im Durchgang (4.22.5, Steel Ball Run gemeldet), Zähler je Staffel mit einer Folge (4.22.6) |
