...
| **Queue: Einleser schreibt jede Staffel einer Sammelseite dem Auftragstitel zu (19.09.2026)** | 3 | JoJo-Sammelseite B0CG7S59KL: Auftrag „Stardust Crusaders“, gemeldet wurden Prime-Staffel 1 (Teil 1), 2 (SC), 3 (Diamond is Unbreakable), 4 (Golden Wind). `fetch-pruefungen.ts` schrieb alle vier als Belege für 20474; von Hand berichtigt. Dauerhaft: Staffel ≠ Auftragsstaffel nur über Folgenzahl/Folgentitel (`prime_folge`) einem Titel der Reihe zuordnen, sonst liegen lassen. JoJo selbst ist erledigt (Teil 1, SC, DiU deutsch; Golden Wind ohne Deutsch, Kanal). |
