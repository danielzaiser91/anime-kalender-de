...
| **31 Crunchyroll-Verweise ohne Urteil — überwiegend Filme** | 5 | Stand 09.09.2026: 33 gemessen, zwei davon am selben Abend geklärt (Fruits Basket: Prelude = deutsch, Gundam Wing Endless Waltz = kein Deutsch im deutschen Katalog). **Zwei Ansätze sind gemessen und tragen kaum:** (1) *Slug statt Titel* — die Adresse trägt denselben Slug, den der Katalog führt; von 33 Verweisen ordnen sich 9 zu, aber **kein einziger Film**: Die Film-Adressen zeigen auf den Slug ihrer **Reihe** („Free! - Iwatobi Swim Club" für fünf Filme), und ein `de-DE` an der Serie belegt den Film nicht. (2) *CR-Suche mit unserem Titel, `type=movie_listing`* — 2 von 33 eindeutig. Der in status.md vermutete Weg über **Jahr und Laufzeit fällt aus**: `discover/browse` liefert weder Laufzeit noch ein Erscheinungsjahr des Werks, nur `series_launch_year` der Katalogaufnahme. **Was bleibt:** Der deutsche Katalog führt 69 Filme (44 mit de-DE) und 1.518 Serien (308 mit de-DE) — die Zuordnung scheitert an unseren Adressen, nicht an der Quelle. Nächster Ansatz wäre der **Beschreibungstext** (er nennt bei „-prelude-" Tohru und Kyo) oder ein Abgleich über die AniList-Kennung, die aniSearch zu vielen CR-Adressen führt |
