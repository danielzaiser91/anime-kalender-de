...
| **Queue (26.09.2026): serena-Probelauf in einer neuen Sitzung** | 2 | serena ist installiert und nur für dieses Repo als MCP-Server eingetragen (Werkzeugkasten). In einer frischen Sitzung messen: Findet und ändert serena Funktionen in `extension/amazon.js` (11.000 Zeilen) mit weniger Kontext als Grep + Read mit Offset? Greift Claude überhaupt darauf zu (serena-Doku warnt, dass Claude Code eigene Werkzeuge bevorzugt)? Ergebnis in `C:\code\ai\__assets\notes\claude-token-sparen.md`, danach behalten oder entfernen |
