...
| Aufgabe | SP | Notiz |
|---|---|---|
| **12 ADN-Verweise ohne Urteil — die Adressschärfung greift bei ihnen nicht** | 3 | Gemessen 07.09.2026. Fünf zeigen auf JoJos Sammelserie 444 (113 von 152 Folgen mit `vde`, also „gemischt" — der Verweis meint eine Staffel, und die steht im Release-Slug). Sieben tragen eine Slug-Adresse ohne Kennung; **sechs davon stehen längst in `data/adn-adressen.yaml`** (eingetragen 06.09.), und trotzdem führt der Datensatz weiter die alte Adresse `animationdigitalnetwork.de/video/<slug>`. Die Schärfung in `build.ts` ist dokumentiert wirksam (38 Adressen berichtigt) — sie erreicht diese hier nicht. Zu klären ist, an welcher Bedingung sie vorbeiläuft; der Rest ist dann Mechanik. Siebter Fall („Plus-Sized Elf") hat keine Kennung und bleibt |
