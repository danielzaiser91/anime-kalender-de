...
| Aufgabe | SP | Notiz |
|---|---|---|
| **Der Sammel-Lauf hinterlässt nichts — Deploy steht seit 06.09., 14:16** | 5 | **Gemessen und reproduziert am 06.09.2026, 19:26.** „Täglich — alle Quellen" endet grün und arbeitet nachweislich (im angestoßenen Lauf 34048467126: „ADN: 121 Tage geprüft, 106 Folgen mit deutscher Synchro"), meldet danach aber **„Keine Änderung an den Quellen — kein Pull Request"**. Seit dem **02.09.** trägt `data/source-health.json` für `adn`, `anime2you` und `crunchyroll` denselben Zeitstempel; die Frist in `check-sources.ts` ist gerissen, `data:check` macht den Bau rot (34047883584, 34038620886, 34029365159), und der Deploy wird übersprungen. **Der Alarm hat also funktioniert — gelesen hat ihn niemand.** <br>**Was schon ausgeschlossen ist:** `data/source-health.json` steht in `QUELLEN` (Eintrag 31 von 82, lokal nachgezählt), sie ist nicht in `.gitignore` (`git check-ignore` findet nichts), `recordSource()` setzt bei **jedem** Aufruf `lastRun: now`, und `writeJson()` schreibt unbedingt. Die Datei müsste sich also bei jedem Lauf ändern. <br>**Wo weiterzusuchen ist:** zwischen Sammelschritt und `tools/quellen-pr.sh` — arbeiten die Schritte auf demselben Arbeitsverzeichnis, und verwirft der Workflow zwischendurch etwas? Dazu der verschluckte Fehler: `git add -- "${QUELLEN[@]}" 2>/dev/null \|\| true` in `quellen-pr.sh` unterdrückt jede Meldung — ein `git status --porcelain` davor würde zeigen, was der Lauf wirklich in der Hand hat |
