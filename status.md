...
| Aufgabe | SP | Notiz |
|---|---|---|
| **erledigt — belegFuer() wählte Adresskorrektur ohne Urteil vor Urteil** | 3 | Lauf 34158655288 („Bestand — zusammenführen und bauen") brach an `check:handbelege` ab: 6 Handbelege (5× My Hero Academia auf Netflix, 1× JoJo no Kimyou na Bouken auf Prime Video) standen falsch im gebauten Datensatz, obwohl `data/dub-confirmed.yaml` korrekt war. Ursache: Für dieselbe Adresse existieren teils zwei Zeilen — eine reine Adresskorrektur ohne `dub`-Urteil und eine echte Urteilszeile ohne (oder mit anderer) `url`. `belegFuer()` in `pipeline/build.ts` bevorzugte bei exakter Adressübereinstimmung jede Zeile, unabhängig davon, ob sie ein Urteil trägt — die urteilslose Korrekturzeile gewann und verdrängte das Urteil. Fix: `belegFuer()` verlangt für den Adressvergleich (und die Adresslos-Rückfallebene) jetzt ein tatsächliches Urteil (`dub` boolean oder `available === false`); erst danach fällt die Funktion auf eine reine Adresskorrektur zurück. Mit `tools/`-Skript gegen die echten `dub-confirmed.yaml`-Daten geprüft: Alle 6 gemeldeten Fälle lösen sich jetzt korrekt auf (5× `dub: false`, 1× `dub: true` samt Adresskorrektur auf `B0CH1GVRKK`). **Zu prüfen bleibt:** dass der nächste „Bestand — zusammenführen und bauen"-Lauf grün durchläuft und `check:handbelege` keine Abweichung mehr meldet — lokal ohne `data/cache/` nicht vollständig baubar. Issue #58, PR folgt |
...
| **35 Crunchyroll-Verweise ohne Urteil — über die Kennung auflösen** | 8 | Daniel am 07.09.2026: „39 Fragezeichen? … wieso funktioniert unser automatismus nicht perfekt". Gemessen: 35 Verweise, davon 23 Filme/OVAs/Specials und 12 Serien mit Adressen im alten Format. **Nicht** die Quelle ist die Lücke — die 69 Filme stehen längst im Katalog (44 mit `de-DE`); Crunchyroll etikettiert nur jeden Eintrag als `typ: series`. Die Lücke ist die Zuordnung: Filme heißen dort anders („-prelude-" statt „Fruits Basket -prelude-"), über den Titel ist genau **einer** von 23 zuzuordnen. **Der gemessene Weg:** `content/v2/cms/objects/<Kennung>?locale=de-DE` löst eine `/watch/<ID>/`-Adresse auf und nennt `audio_locale` des Videos — an sechs Fällen geprüft, drei lösen sich auf (given: `de-DE`, zwei Rascal-Filme: `ja-JP`), drei antworten 404 (abgelaufene Kennung, der Verweis ist tot). **Achtung bei der Auswertung:** `de-DE` ist ein sicheres Ja, `ja-JP` ist **kein** Nein — die deutsche Fassung hat eine eigene Videokennung. Ein Nein entsteht erst, wenn zusätzlich der Serieneintrag im Katalog kein `de-DE` führt (sonst wiederholen sich die 975 Falschangaben vom 15.08.2026). Einzelheiten in `CLAUDE.md`, „Die Crunchyroll-Fragezeichen liegen an der Zuordnung" |
