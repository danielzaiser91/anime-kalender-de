...
| Aufgabe | SP | Notiz |
| **Erledigt am 21.09.2026: Titel-Verlust-Riegel bricht bei legitimem Verschieben hinter den Toggle ab (Lauf 35550446358)** | 1 | „Black Clover 2nd Season" (195604) und „Kusuriya no Hitorigoto 3rd Season" (195516) verloren ihren einzigen (unbestätigten) Crunchyroll-Termin und wanderten korrekt hinter den Toggle (JP-Start noch aus, keine Sprechrollen) — der Verlust-Riegel in `pipeline/build.ts` kannte die Zusicherung aus `schreibeOhneSynchro` nicht und wertete das als Datenverlust. Behoben (PR folgt, Issue #186): Der Riegel zieht IDs aus der `verschoben`-Liste jetzt von der Verlust-Zählung ab. Zu prüfen bleibt der nächste reguläre Bau-Lauf — grün, wenn beide Titel in `ohne-synchro.json` landen statt den Bau erneut abzubrechen. |
...
| **Offen: 330 Amazon-Verweise ohne Linkbefund — Schübe mit Stunden Abstand** | 2 | Stand 20.09.2026, 15:20 (gezählt in `data/link-check.json`): 1.730 Amazon-Adressen, davon 1.344 mit 200, 43 Regionssperre, 12 mit 503, 1 mit 404, **330 unklar**. **Gemessen an diesem Tag:** Amazons Abwehr hängt an der Menge je Zeitfenster, nicht am Takt — 669 Abrufe bei 700 ms, aber nur 20 bei 3 s Takt direkt nach einer Sperre; eine Viertelstunde Ruhe stellt nichts wieder her (20, 20, 20, 91). Vorgehen deshalb: ein Schub von höchstens rund 600 Adressen (`npx tsx pipeline/check-links.ts --alter 3 --limit 600 --pause 700`), dann Stunden Ruhe. Rückfallebene ist `tavily_extract` (gemessen, aber noch ohne Gegenprobe). |
