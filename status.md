...
| **Space Dandy: Staffel 2 ohne Folgen, Knopf will 26 aus Staffel 1 melden** | 3 | Bericht in `docs/diagnose/space-dandy-staffel2-ohne-folgen.json` (4.0.26, 31.08.2026, 01:08). **Derselbe Kern wie bei Shangri-La** — `letzteKennung: "2|1|B0CJL3MC84|Space Dandy"`, also Adresse 2 gegen Seite 1, `quelltextVeraltet: false`. **Dazu eine Folge, die schwerer wiegt:** Die Seite zeigt für Staffel 2 gar nichts (`folgenImDom: 0`, `folgenLautSeite: null`, `hatFolgenReiter: false`), der Zählstand trägt aber weiter `gesamt: 26, nummern: 26` — **und zwar unter der neuen Adresse** (`fuerAdresse: "…?ref_=atv_dp_season_select_s2"`). Der Stand der alten Staffel ist also auf die neue Adresse gewandert; genau das soll `fuerAdresse` verhindern. Daniel hatte Staffel 1 bereits gemeldet und wollte nur wechseln. Zu prüfen: Wird der Stand beim Wechsel neu gestempelt statt verworfen, oder trägt die Nachzügler-Antwort des Mitlesers schon die neue Adresse? |
| **Shangri-La Frontier: Staffel 2 gewählt, Knopf sagt Staffel 1** | 2 | Bericht liegt in `docs/diagnose/shangri-la-staffel2-als-staffel1.json` (4.0.26, 31.08.2026, 01:06). Gemessen: Adresse `?ref_=atv_dp_season_select_s2`, Auswahlfeld „Shangri-La Frontier Staffel 2", Seitentext „Season 2", `lage.staffelZahl: 2` — aber `zustand.staffelNummer: 1` und `letzteKennung: "2|1|B0D4YZTX2S|…"`, also Adresse 2 gegen Seite 1. **`quelltextVeraltet: false`**, der Block ist demnach frisch; `gabStaffelwechsel: true`, `gemeldeterBand: undefined`. Seit 2.8 gilt `headerDetail.seasonNumber` vor der Adresse (Yu-Gi-Oh! ZEXAL: Adresse trug 101 für Band 1). Hier liefert der Block offenbar 1, obwohl Staffel 2 gewählt ist — zu prüfen: Kommt die 1 aus dem `headerDetail` der Serienseite statt der Staffel, und trägt der gewählte Eintrag der Staffelliste (`isSelected`) die richtige Nummer? Der Fall ist wichtig, weil die Meldung sonst unter der falschen Staffel landet. |
| **Prime-Suche mit dem deutschen Titel, ohne Gattungswörter** | 3 | Gemessen am 31.08.2026: Von 51 Prime-Suchadressen tragen **26 den englischen** Titel, 24 den deutschen. Der englische findet oft nichts — „Sailor Moon S Movie: Hearts in Ice" gibt bei Prime null Anime-Treffer, obwohl der Film dort unter „Sailor Moon S: Schneeprinzessin Kaguya" liegt (Daniel, mit Bild aus aniSearch). **Drei Teile:** (1) Suchbegriff = `titleDe`, Englisch nur als Rückfall. (2) Gattungswörter und Satzzeichen raus — Ziel für diesen Fall: `Sailor Moon S Schneeprinzessin Kaguya`, also ohne „Movie" und ohne Doppelpunkt. Die Liste steht schon in `extension/amazon.js` als `GATTUNG`; sie gehört in ein gemeinsames Modul, damit Suche und Abgleich dieselbe benutzen. (3) Der Knopf „Anders schreiben" wird ersetzt durch einen mit der **englischen** Schreibweise — er ist der Rückfall, wenn Prime den deutschen Titel nicht führt. Erzeugt werden die Adressen in `pipeline/fetch-rohfolgen.ts` (Zeile 140) und im Bau; die Prüfliste liest sie aus `titles.json`. |
